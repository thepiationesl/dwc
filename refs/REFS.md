...
> 用途：为 dwc 镜像脚本开发提供参考。所有脚本落笔前先对照本清单，避免踩老版本/过时方案的坑。
> 更新日期：2026-08-10
...
### 1. infrastlabs/docker-headless（重点，你已参考）
...
- **架构设计值得抄**：
  - `slim > base > 各桌面` 的镜像分层（但不违反你"不复用镜像"原则，可照抄思路用 COPY 共享脚本）
...
- **坑（版本老，别照抄）**：
  ...
  - 提供 `docker-dind`，需要特权，与你的无特权设计冲突

### 2. hackerschoice/segfault（你已参考）
...
### 3. kmille36/Docker-Kali-Desktop-NoMachine（与你的 full/studio 场景最贴近）
...
- **mscrnt/ubuntu-desktop-docker**：Ubuntu24.04 XFCE，**systemd PID1**，VNC+xrdp+SSH，可选 NoMachine + NVIDIA，cgroup v2 无特权（与你的无特权要求一致）
...
### 虚拟化类（对照你的 vm）
- **dockur/** 系列：QEMU/KVM 虚拟机容器（windows/macos/linux），不自由、配置不可热改 —— 正是你想避开的
